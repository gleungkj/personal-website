...
export const PageTable: React.FC<IPageTableProps> = ({
  ...
  useEffect(() => {
    ...
    animate("div", {opacity: 1, x: 0, y: 0}, {delay: stagger(0.25), duration: 1})
  ...
  return (
    <div className={styles.table} ref={scope}>
      {websiteContents.map((content) => (
        <div key={content.id}>
          <motion.div initial={{opacity:0, y: "50%"}} animate={{y: 0}} transition={{ease:'easeInOut'}} className={styles.field}>{content.field}
          </motion.div>
          <motion.div initial={{opacity:0, x: "-50%"}} animate={{x: 0}} transition={{ease:'easeInOut'}} className={styles.content}>{content.content}
          </motion.div>
